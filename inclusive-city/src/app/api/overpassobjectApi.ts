...
export class RoutingClient extends ApiBase {
    ...
    getComputedRoute(originLatitude: number | undefined, originLongitude: number | undefined, destinationLatitude: number | undefined, destinationLongitude: number | undefined): Promise<OsrmRouteDto> {
    ...
    protected processGetComputedRoute(response: Response): Promise<OsrmRouteDto> {
        ...
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as OsrmRouteDto;
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
...
export class StructureClient extends ApiBase {
    ...
    protected processGetStructures(response: Response): Promise<GetStructuresDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as GetStructuresDto;
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
